'''
    Programming DConvnet classes courtesy of: 
    # Author: Liang Jiang
    ...
    1. Callback
    2. DeConvnet
    3. utility functions(MNIST Dataset)
    4. utility functions(RPS Dataset)
'''
import os
import numpy as np
import sys
...
class DConvolution2D(object):
    ...
    def up(self, data, learning_phase = 0):
        ...
        ### can be not compatible with numpy versions like 1.18.1
...
def load_images(train=True):
    ...
    # not very elegant, but works anyway.
    for i in range(img_rows):
        for j in range(img_cols):
            mean_train = np.mean(x_train[:, i, j, :])
            std_train = np.std(x_train[:, i, j, :])
            mean_test = np.mean(x_test[:, i, j, :])
            std_test = np.std(x_test[:, i, j, :])

            if std_train == 0:
                x_train[:, i, j, :] = 0
            else:
                x_train[:, i, j, :] = (x_train[:, i, j, :] - mean_train)/std_train
            if std_test == 0:
                x_test[:, i, j, :] = 0
            else:
                x_test[:, i, j, :] = (x_test[:, i, j, :] - mean_test)/std_test
    ...
    # not very efficient, but let's just keep it this way:
    if train:
